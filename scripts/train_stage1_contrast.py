...
from cabria.engine.stage1_contrast_trainer import train_stage1_contrast
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train CABRIA bridge-support contrast representation.")
    ...
    tracker = init_tracker(config, job_type="train_stage1_contrast") if is_main else None
    try:
        checkpoint = train_stage1_contrast(config, data_config, device, tracker=tracker)
        if is_main and tracker is not None:
            tracker.log_summary({"artifacts/final_stage1_contrast_checkpoint": checkpoint.as_posix()})
        if is_main:
